...
#define SOFTHARD_TMPVAL 4.4e-8
            ...
            //         SOFTHARD_TMPVAL being used for now so this can still be merged into master alone.
            pattern->radius = std::max(Parse_Float(),SOFTHARD_TMPVAL)+EPSILON;
...
#define SOFTHARD_TMPVAL 4.4e-8
            ...
            //         SOFTHARD_TMPVAL being used for now so this can still be merged into master alone.
            pattern->spacing = std::max(Parse_Float(),SOFTHARD_TMPVAL) + EPSILON;
#undef SOFTHARD_TMPVAL
        END_CASE
        CASE (STRENGTH_TOKEN)
            pattern->strength = std::max(Parse_Float(),EPSILON);
        END_CASE
        OTHERWISE
            UNGET
            EXIT
