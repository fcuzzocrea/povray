...
boost::posix_time::ptime Parser::Get_File_Time(shared_ptr<BackendSceneData>& sd, const UCS2String& filename, unsigned int stype, bool err_flag)
{
    UCS2String fn(filename);
    UCS2String foundfile(sd->FindFile(GetPOVMSContext(), fn, stype));
    ...
    return sd->GetFileTime(GetPOVMSContext(), fn, foundfile.c_str(), stype);
}
